...
@push('scripts')
    <script>
        let table_data = @json($performance_automation_inbox);
    </script>
@endpush
@section('content')
    <div class="page pt-3">
        <div class="w-100 content-window bg-menu rounded border">
            <div class="w-100 iransans p-3 border-bottom d-flex flex-row align-items-center justify-content-between">
                <div>
                    ...
                    <h5 class="iransans d-inline-block m-0">اتوماسیون کارکرد ماهانه پرسنل</h5>
                    ...
                    <div style="overflow: hidden" class="p-3">
                        <div id="table-scroll" class="table-scroll">
                            ...
                            <tr v-for="record in table_data_records">
                                ...
                                <td><span class="iranyekan">@{{ record.performances.length }}</span></td>
                                ...
                                <td>
                                    <div class="dropdown table-functions">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            @can('details','PerformanceAutomation')
                                            ...
                                            @endcan
                                            @can('disagree','PerformanceAutomation')
                ...
                <div class="tab-pane fade" id="outbox" role="tabpanel" aria-labelledby="outbox-tab">
                    ...
                    <div style="overflow: hidden" class="p-3">
                        <div id="table-scroll" class="table-scroll">
                            ...
                            <tbody>
                            @forelse($performance_automation_outbox as $performance_outbox)
                                <tr>
                                    <td><span class="iranyekan">{{ $performance_outbox->id }}</span></td>
                                    <td><span class="iranyekan">{{ "{$performance_outbox->contract->name}({$performance_outbox->contract->workplace})" }}</span></td>
                                    <td><span class="iranyekan">{{ $performance_outbox->authorized_date->automation_year }}</span></td>
                                    <td><span class="iranyekan">{{ $performance_outbox->authorized_date->month_name }}</span></td>
                                    <td><span class="iranyekan">{{ $performance_outbox->current_role->name }}</span></td>
                                    <td>
                                        @if($performance_outbox->is_finished == 0 && $performance_outbox->is_committed == 0)
                                            <span class="iranyekan">منتظر ارسال</span>
                                        @elseif($performance_outbox->is_finished == 0 && $performance_outbox->is_committed == 1)
                                    ...
                                    <td>
                                        <div class="dropdown table-functions">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                @can('details','PerformanceAutomation')
                                                    <a role="button" href="{{ route("PerformanceAutomation.details",[$performance_outbox->id,true]) }}" class="dropdown-item">
                                                        <i class="fa fa-list-ul"></i>
                                                        <span class="iranyekan">مشاهده جزئیات</span>
                                                    </a>
                                                @endcan
