...
@push('scripts')
    <script>
        let table_data = @json($advantage_automation_inbox);
    </script>
@endpush
@section('content')
    <div class="page pt-3">
        <div class="w-100 content-window bg-menu rounded border">
            <div class="w-100 iransans p-3 border-bottom d-flex flex-row align-items-center justify-content-between">
                <div>
                    ...
                    <h5 class="iransans d-inline-block m-0">اتوماسیون تغییرات مزایا پرسنل</h5>
                    ...
                    <div class="table-responsive p-3" style="min: auto" v-cloak>
                        <table id="inbox_table" class="table table-striped static-table" data-filter="[1,2,3]">
                            ...
                            <tr v-for="record in table_data_records">
                                ...
                                <td><span class="iranyekan">@{{ record.employee.first_name + " " + record.employee.last_name }}</span></td>
                                <td><span class="iranyekan">@{{ record.employee.national_code }}</span></td>
                                <td><span class="iranyekan">@{{ record.contract }}</span></td>
                                <td><span class="iranyekan">@{{ record.advantage.name }}</span></td>
                                <td><span class="iranyekan">@{{ record.type === "add" ? "اضافه" : "حذف" }}</span></td>
                                ...
                                <td>
                                    <div class="dropdown table-functions">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            @can('details','AdvantageAutomation')
                                            ...
                                            @endcan
                                            @can('disagree','AdvantageAutomation')
                ...
                <div class="tab-pane fade" id="outbox" role="tabpanel" aria-labelledby="outbox-tab">
                    ...
                    <div class="table-responsive p-3" style="min: auto">
                        <table id="outbox_table" class="table table-striped static-table" data-filter="[1,2,3]">
                            ...
                            <tbody>
                            @forelse($advantage_automation_outbox as $advantage_outbox)
                                <tr>
                                    <td><span class="iranyekan">{{ $advantage_outbox->id }}</span></td>
                                    <td><span class="iranyekan">{{ "{$advantage_outbox->employee->first_name} {$advantage_outbox->employee->last_name}" }}</span></td>
                                    <td><span class="iranyekan">{{ $advantage_outbox->employee->national_code }}</span></td>
                                    <td><span class="iranyekan">{{ $advantage_outbox->contract }}</span></td>
                                    <td><span class="iranyekan">{{ $advantage_outbox->advantage->name }}</span></td>
                                    <td><span class="iranyekan">{{ $advantage_outbox->type == "add" ? "اضافه" : "حذف"}}</span></td>
                                    <td><span class="iranyekan">{{ $advantage_outbox->current_role->name }}</span></td>
                                    <td>
                                        @if($advantage_outbox->is_finished == 0 && $advantage_outbox->is_committed == 0)
                                            <span class="iranyekan">منتظر ارسال</span>
                                        @elseif($advantage_outbox->is_finished == 0 && $advantage_outbox->is_committed == 1)
                                    ...
                                    <td>
                                        <div class="dropdown table-functions">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                @can('details','AdvantageAutomation')
                                                    <a role="button" href="{{ route("AdvantageAutomation.details",$advantage_outbox->id) }}" class="dropdown-item">
                                                        <i class="fa fa-list-ul"></i>
                                                        <span class="iranyekan">مشاهده جزئیات</span>
                                                    </a>
                                                @endcan
