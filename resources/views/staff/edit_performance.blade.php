        ...
        <div class="w-100 content-window bg-menu rounded border">
            ...
            <div class="page-header">
                <div class="input-group">
                    <div class="input-group-prepend" style="border-radius: 0">
                        <a href="{{ route("Performances.performance_export_excel",[$contract_subset->id,$authorized_date["id"]]) }}" class="btn btn-outline-info mr-2" style="border-radius: 0.25rem">
                    ...
                    <input type="text" class="form-control iranyekan" readonly value="{{ "کارکرد ماهانه پرسنل ".$automation->contract->name."(".$contract_subset->workplace.") ".$authorized_date["month_name"]." ماه سال ".$authorized_date["automation_year"] }}">
            ...
            <div style="overflow: hidden" class="p-3">
                <div id="table-scroll" class="table-scroll">
                    ...
                    <tr v-for="item in table_data_records.performances">
                        ...
                        <td v-for="(attribute,index) in table_data_records.attributes.items" style="min-width: 90px"><input class="form-control iranyekan text-center" :type="attribute['kind']" min="0" :value="get_employee_performance_value(item.employee_id,index)" v-on:input="set_employee_performance_value($event,item.employee_id,index)"></td>
                    </tr>
                    </tbody>
                </table>
            </div>
            </div>
            <form id="main_submit_form" class="p-3" action="{{ route("Performances.update",$automation->id) }}" data-json="employees_data" method="post" v-on:submit="submit_form">
                @csrf
                @method("put")
                ...
                <div class="form-row">
                    <div class="form-group col-12">
                        ...
                        <textarea class="form-control iranyekan w-100" style="height: 80px" name="comment"></textarea>
                    </div>
                </div>
            </form>
            <div class="w-100 form-button-row text-center pt-4 pb-4 position-sticky" style="z-index: 1000;bottom: 0">
                <button class="btn btn-success submit_button" data-method="edit" v-on:click="performance_validation">
                    ...
                    <span class="iranyekan">ویرایش کارکرد</span>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <axios-button :class="'btn btn-outline-primary'" :route="'{{ route("PerformancePreImport") }}'" :action="'load'" :required="['#upload_file']" :elements="['#upload_file','#contract_id','#authorized_date_id']"  :message="'آیا برای بارگذاری فایل کارکرد اطمینان دارید؟'">
