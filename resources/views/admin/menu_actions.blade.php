    ...
    <div class="page pt-3">
        <div class="w-100 content-window bg-menu rounded border">
            <div class="w-100 iransans p-3 border-bottom d-flex flex-row align-items-center justify-content-between">
                <div>
                    <i class="fa fa-bolt-lightning fa-1-4x mr-1"></i>
            ...
            <div class="page-header">
                <div class="input-group">
                    <div class="input-group-prepend">
                        <button class="btn btn-outline-info mr-2" data-toggle="modal" data-target="#new_menu_action_modal">
                            <i class="fa fa-plus-circle fa-1-2x mr-1"></i>
                            <span class="iransans create-button">عملیات جدید</span>
                        </button>
                    </div>
                    <input type="text" class="form-control text-center iranyekan"data-table="main_table" placeholder="جستجو با نام و عملیات" v-on:input="filter_table">
                    <div class="input-group-prepend">
            ...
            <div id="table_responsive" class="table-responsive p-3">
                <table id="main_table" class="table table-striped static-table" data-filter="[1,2]">
                    ...
                    @forelse($menu_actions as $menu_action)
                        <tr>
                            <td><span class="iranyekan">{{$menu_action->id}}</span></td>
                            <td><span class="iranyekan">{{$menu_action->name}}</span></td>
                            <td><span class="iranyekan">{{$menu_action->action}}</span></td>
                            <td><span class="iranyekan">{{$menu_action->user->name}}</span></td>
                            <td><span class="iranyekan">{{verta($menu_action->created_at)->format("H:i:s Y/m/d")}}</span></td>
                            <td><span class="iranyekan">{{verta($menu_action->updated_at)->format("H:i:s Y/m/d")}}</span></td>
                            <td>
                                <div class="dropdown table-functions iranyekan">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <a role="button" href="{{ route("MenuActions.edit",$menu_action->id) }}" class="dropdown-item">
                                            <i class="fa fa-edit"></i>
                                            <span class="iranyekan">ویرایش</span>
                                        </a>
                                        <div class="dropdown-divider"></div>
                                        <form class="w-100" id="delete-form-{{ $menu_action->id }}" action="{{ route("MenuActions.destroy",$menu_action->id) }}" method="POST" v-on:submit="submit_form">
                                            @csrf
                                            @method("Delete")
                                            <button type="submit" form="delete-form-{{ $menu_action->id }}" class="dropdown-item">
...
@endsection
@section('modals')
    <div class="modal fade rtl" id="new_menu_action_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title iransans">ایجاد عملیات جدید</h6>
                </div>
                <div class="modal-body">
                    <form id="main_submit_form" action="{{route("MenuActions.store")}}" method="post" data-type="create" v-on:submit="submit_form">
                        @csrf
                        <div class="form-row">
                            ...
                            <div class="form-group col-12">
                                ...
                                <input type="text" class="form-control text-center iranyekan @error('action') is-invalid @enderror" name="action" value="{{old("action")}}">
                                @error('action')
