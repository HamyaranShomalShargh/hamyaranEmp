...
class StaffUserRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        elseif ($this->method() == "PUT"){
            return [
                'name' => ['required'],
                'username' => ['required', 'string', 'min:8', 'unique:users,username,'.$this->route('StaffUser').",id"],
                'role_id' => ['required'],
                'password' => ['sometimes','nullable', 'min:8', 'confirmed'],
                'email' => ['required', 'string', 'email', 'max:255', 'unique:users,email,'.$this->route('StaffUser').",id"],
                "mobile" => ["required", "regex:/^09(1[0-9]|9[0-2]|2[0-2]|0[1-5]|41|3[0,3,5-9])\d{7}$/", "unique:users,mobile,".$this->route('StaffUser').",id"],
                "contracts" => "required",
                "upload_file" => "sometimes|nullable|mimes:png"
            ];
        }
    ...
    public function messages(): array
    {
        return [
            ...
            "role_id.required" => "انتخاب عنوان شغلی الزامی می باشد",
            ...
            "contracts.required" => "انتخاب حداقل یک قرارداد مجاز، الزامی می باشد",
