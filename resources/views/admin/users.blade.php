    ...
    <div class="page w-100 pt-3">
        <div class="w-100 content-window bg-menu rounded border">
            <div class="w-100 iransans p-3 border-bottom d-flex flex-row align-items-center justify-content-between">
                <div>
                    <i class="fa fa-users fa-1-4x mr-1"></i>
            ...
            <div class="page-header">
                <div class="input-group">
                    ...
                    <input type="text" class="form-control text-center iranyekan" data-table="main_table" placeholder="جستجو با نام ، نام کاربری و عنوان شغلی" v-on:input="filter_table">
                    <div class="input-group-prepend">
            ...
            <div id="table_responsive" class="table-responsive p-3">
                <table id="main_table" class="table table-striped static-table" data-filter="[1,2,3]">
                            ...
                            <td>
                                <div class="dropdown table-functions iranyekan">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <form class="w-100" id="activation-form-{{ $user->id }}" action="{{ route("Users.activation",$user->id) }}" method="POST" v-on:submit="submit_form">
                                            @csrf
                                        ...
                                        <a role="button" href="{{ route("Users.edit",$user->id) }}" class="dropdown-item">
                                            <i class="fa fa-edit"></i>
                                            <span class="iranyekan">ویرایش</span>
                                        </a>
                                        <div class="dropdown-divider"></div>
                                        <a role="button" href="{{ route("Users.report",$user->id) }}" class="dropdown-item">
                                            <i class="fa fa-list-check"></i>
                                            <span class="iranyekan">گزارشات(log)</span>
                                        </a>
                                        <div class="dropdown-divider"></div>
                                        <form class="w-100" id="report-delete-form-{{ $user->id }}" action="{{ route("Users.report.delete",$user->id) }}" method="POST" v-on:submit="submit_form">
                                            @csrf
                                            @method("Delete")
                                            <button type="submit" form="report-delete-form-{{ $user->id }}" class="dropdown-item">
                                                <i class="fa fa-trash"></i>
                                                <span class="iranyekan">حذف کلی گزارشات</span>
                                            </button>
                                        </form>
                                        <div class="dropdown-divider"></div>
                                        <form class="w-100" id="delete-form-{{ $user->id }}" action="{{ route("Users.destroy",$user->id) }}" method="POST" v-on:submit="submit_form">
                                            @csrf
                                            @method("Delete")
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="main_submit_form" action="{{route("Users.store")}}" method="post" data-type="create" v-on:submit="submit_form" enctype="multipart/form-data">
                        @csrf
                        <div class="form-row">
